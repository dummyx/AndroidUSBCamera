...
class AV1EncodeProcessor(
    ...
    override fun handleStartEncode() {
        try {
            ...
            doEncodeData()
            Logger.i(TAG, "init av1 media codec success, bit = ")

        } catch (e: Exception) {
            Logger.e(TAG, "start av1 media codec failed, err = ${e.localizedMessage}", e)
        }
    }
    ...
    override fun handleStopEncode() {
        try {
            ...
            mMediaCodec?.stop()
            mMediaCodec?.release()
            Logger.i(TAG, "release av1 media codec success.")
        } catch (e: Exception) {
    ...
    override fun processOutputData(
        ...
        return Pair(IEncodeDataCallBack.DataType.AV1, encodeData)
    }
